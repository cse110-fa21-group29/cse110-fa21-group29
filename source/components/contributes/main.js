window.addEventListener('DOMContentLoaded', () => {
    ...
    let ingredientbox = document.getElementById("ingredients-input-grid").children
    let i1 = ingredientbox[2]
    let i2 = ingredientbox[4]
    let i3 = ingredientbox[6]
    ingredientbox[0].addEventListener("click", () => { addIngredients() })
    ingredientbox[2].addEventListener("click", () => { deleteIngredient(i1) })
    ingredientbox[2].previousSibling.remove()
    ingredientbox[4].addEventListener("click", () => { deleteIngredient(i2) })
    ingredientbox[4].previousSibling.remove()
    ingredientbox[6].addEventListener("click", () => { deleteIngredient(i3) })
    ...
    let s1 = steps[1]
    let s2 = steps[2]
    steps[0].addEventListener("click", () => { addSteps() })
    steps[1].addEventListener("click", () => { s1.remove() })
    steps[2].addEventListener("click", () => { s2.remove() })
...
function addIngredients() {
    ...
    newbutton.addEventListener("click", () => { deleteIngredient(newbutton) })
}
...
function addSteps() {
    ...
    FilePond.parse(document.body)
    stepdeletebutton.addEventListener("click", () => { stepgrid.remove() })
}
